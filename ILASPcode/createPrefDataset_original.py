...
NNoutput_dir = "../Ordinamenti/"
zero_dir_train = "Data8Component2Std/users_original/zero/train/150Couples/"
no_zero_dir_train = "Data8Component2Std/users_original/no_zero/train/150Couples/"
zero_dir_test = "Data8Component2Std/users_original/zero/test/50Couples/"
no_zero_dir_test = "Data8Component2Std/users_original/no_zero/test/50Couples/"
f_preferences_train = os.path.join(NNoutput_dir, 'dataset_coppie.txt')
f_labels_train = os.path.join(NNoutput_dir, 'output-file.txt')
# f_preferences_test = os.path.join(NNoutput_dir, 'output-file.txt')
# f_labels_test = os.path.join(NNoutput_dir, 'test/50Couples/labels.txt')
...
# couples_dataset_test = np.zeros((len(linesOfPTest)-1, 210, 2), dtype='int32')
...
# couple_label_test = np.zeros((len(linesOfLTest)-1, 50), dtype='int32')
...
for i in range(48, 54):
    couples_dataset_train, couples_dataset_test, couple_label_train, couple_label_test = train_test_split(couples_dataset[i], couple_label[i], shuffle=True, test_size=53, random_state=42)
    ...
    sys.stdout = open(f_user_output, 'w')
    counter = 0
    for j, couple in enumerate(couples_dataset_train):
        if couple_label_train[j] == 1:
            print("#brave_ordering(o" + str(counter + 1) + "@1, item" + str(couple[0]) + ", item" + str(couple[1]) + ", <).")
            counter = counter + 1
        elif couple_label_train[j] == -1:
            print("#brave_ordering(o" + str(counter + 1) + "@1, item" + str(couple[0]) + ", item" + str(couple[1]) + ", >).")
            counter = counter + 1
        else:
            print("#brave_ordering(o" + str(counter + 1) + "@1, item" + str(couple[0]) + ", item" + str(couple[1]) + ", =).")
    ...
    sys.stdout = open(f_user_output, 'w')
    counter = 0
    for j, couple in enumerate(couples_dataset_test):
        if couple_label_test[j] == 1:
            print("#brave_ordering(o" + str(counter + 1) + "@1, item" + str(couple[0]) + ", item" + str(couple[1]) + ", <).")
            counter = counter + 1
        elif couple_label_test[j] == -1:
            print("#brave_ordering(o" + str(counter + 1) + "@1, item" + str(couple[0]) + ", item" + str(couple[1]) + ", >).")
            counter = counter + 1
        else:
            print("#brave_ordering(o" + str(counter + 1) + "@1, item" + str(couple[0]) + ", item" + str(couple[1]) + ", =).")
