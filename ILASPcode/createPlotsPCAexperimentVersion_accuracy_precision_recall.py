            ...
            if scope == "":
                for i, user_matrix in enumerate(data_45_couples_no_zeros):
                    if np.all(user_matrix == 0):
                        ...
                        plt.savefig("./PCAexperiment/plots" + scope + str(PCAindex) +"/percentages/45Couple/user" + str(i) +".png", dpi=300)
                        fig, ax = plt.subplots()
                        ...
                        plt.savefig("./PCAexperiment/plots" + scope + str(PCAindex) +"/training_time/45Couple/user" + str(i) +".png", dpi=300)
                ...
                for i, user_matrix in enumerate(data_105_couples_no_zeros):
                    if np.all(user_matrix == 0):
                        ...
                        plt.savefig("./PCAexperiment/plots" + scope + str(PCAindex) +"/percentages/105Couple/user" + str(i) +".png", dpi=300)
                        fig, ax = plt.subplots()
                        ax.plot(x, train_times, label="training time")
                        ax.set_xlabel('max_v and max_p with max_v == max_p')
                        ax.set_ylabel('time in second')
                        ax.set_title('user' + str(i) + "-105Couples-Zeros")
                        ax.legend(loc="lower right")
                        plt.savefig("./PCAexperiment/plots" + scope + str(PCAindex) +"/training_time/105Couple/user" + str(i) +".png", dpi=300)
