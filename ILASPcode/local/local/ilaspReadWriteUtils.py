...
def preferencesFromFileSpacesAndSignSampledCouples(path):
    preferences = []
    with open(path) as f:
        for line in f:
            match = re.match('#brave_ordering\(.*,\s+([a-zA-Z]+\d+s\d+),\s+([a-zA-Z]+\d+s\d+),\s+([<>=]).*', line)
            if match: preferences.append((match.groups()[0], match.groups()[1], match.groups()[2]))
...
def test_cm(theory, items, test_set):
    ...
    #   u u  =  |   8   |   9   |   7      class 0
    #   t t  <  |   2   |   3   |   1      class 1
    # NOTE: label > here is intended as ILASP syntax in brave ordering, and so a > b means that b is strictly preferred over a (cause it activates a greater amount of weights from wc)
    #       similarly a < b means that a is strictly preferred over b. https://doc.ilasp.com/specification/cdoes.html
    for preference in test_set:
...
def test_cm_number(theory, items, test_set):
    # just used for local testTheory
    return compare_cm(items[test_set[0][0]], items[test_set[0][1]], test_set[0][2], theory)
